function [X,U] = reconstruct2D(phys,u,order)
...
%   order = Order of approximation
    ...
    X = zeros(2,Ne,Np);
    for i = 1:Np
        X(:,:,i) = reshape(elco*l(:,i),2,[]);
    end
    elu = ofem_v2.tools.matrixarray(reshape(u(phys.DOFs.el2DOF(:,:)'),[],1,Ne));
    for q = 1:Np
        phi(:,:,1) = phys.element.N{1}(l(1,q),l(2,q));
        phi(:,:,2) = phys.element.N{2}(l(1,q),l(2,q));
        phi = ofem_v2.tools.matrixarray(phi(:,:,phys.geometry.refTet));
        U(:,:,q) = reshape((DinvT*phi)*elu,2,[]);
    end
