...
%             validateattributes(A,{'ofem.matrixarray'},{});
        ...
        function C=dot(A,B,varargin)
            C=ofem_v2.tools.matrixarray(dot(double(A),double(B),varargin{:}));
        end

        %%
        function C=reshape(A,varargin)
            C=ofem_v2.tools.matrixarray(reshape(double(A),varargin{:}));
        end

        %%
        function C=cross(A,B,varargin)
            C=ofem_v2.tools.matrixarray(cross(double(A),double(B),varargin{:}));
        end

        %%
        function C=conj(obj)
            C=ofem_v2.tools.matrixarray(conj(double(obj)));
        end
        ...
        function C=plus(A,B)
            ...
            if isscalar(A) || isscalar(B) || (numel(size(A))==numel(size(B)) && all(size(A)==size(B)))
                C=ofem_v2.tools.matrixarray(double(A)+double(B));
            else
                if size(A,3)==size(B,3)
                    if isscalar(A(:,:,1))
                        C=ofem_v2.tools.matrixarray(repmat(double(A),size(B(:,:,1)))+double(B));
                    elseif isscalar(B(:,:,1))
                        C=ofem_v2.tools.matrixarray(double(A)+repmat(double(B),size(A(:,:,1))));
...
%                     elseif size(A(:,:,1))==size(B(:,:,1))
                    ...
                    if size(A,3)==1
                        C=ofem_v2.tools.matrixarray(repmat(double(A),1,1,size(B,3))+double(B));
                    elseif size(B,3)==1
                        C=ofem_v2.tools.matrixarray(double(A)+repmat(double(B),1,1,size(A,3)));
                    else
        ...
        %%
        function C=uplus(A)
            C=ofem_v2.tools.matrixarray(+double(A));
        end
        ...
        function C=times(A,B)
            ...
            if isscalar(A) || isscalar(B) || (numel(size(A))==numel(size(B)) && all(size(A)==size(B)))
                ...
                if size(A,3)==size(B,3)
                    ...
                    elseif isscalar(B(:,:,1))
                        C=ofem_v2.tools.matrixarray(double(A).*repmat(double(B),size(A(:,:,1))));
                    elseif size(A,1)==size(B,1) && size(B,2) == 1
                        C = ofem_v2.tools.matrixarray(double(A).*repmat(double(B),size(A(1,:,1))));
                    elseif size(A,1)==size(B,1) && size(A,2) == 1
                        C = ofem_v2.tools.matrixarray(double(B).*repmat(double(A),size(B(1,:,1))));
                    elseif size(A,2) == size(B,2) && size(A,1) == 1
                        C = ofem_v2.tools.matrixarray(double(B).*repmat(double(A),size(B(:,1,1))));
                    elseif size(A,2) == size(B,2) && size(B,1) == 1
                        C = ofem_v2.tools.matrixarray(double(A).*repmat(double(B),size(A(:,1,1))));
                    else
                        error('ofem:matrixarray:DimensionMismatch', ...
                              'Each array entry must either be a scalar or dimensions must match');
        ...
        function C=mtimes(A,B)
            ...
            if ismatrix(A)
                ...
                C=ofem_v2.tools.matrixarray(reshape(tmp,NrA,NcB,[]));
                return;
            elseif ismatrix(B)
                NrA=size(A,1);
                NcA=size(A,2);
                ...
                C=ofem_v2.tools.matrixarray(permute(reshape(tmp,NrA,[],NcB),[1,3,2]));
                return;
            end
            ...
            C=ofem_v2.tools.matrixarray(reshape(tmp,NA,NB,[]));
        end
